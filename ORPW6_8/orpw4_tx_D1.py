...
def build_uart_frame(channels: list[int]) -> bytes:
    ...
    for b in payload:
        crc ^= b
        frame = bytes([FRAME_SOF]) + payload + bytes([crc, FRAME_EOF])

    # DEBUG — pokaż całą ramkę hex
    print(f"[TX] {frame.hex()}")
    # print(
    #  f"[TX] len={len(frame)} SOF={frame[0]:#04x} CRC={frame[25]:#04x} EOF={frame[26]:#04x}"
    # )
    return frame
    ...
    def __init__(self):
        ...
        try:
            self._ser = serial.Serial(
                UART_PORT, UART_BAUD, bytesize=8, parity="E", stopbits=1, timeout=0.01
            )
