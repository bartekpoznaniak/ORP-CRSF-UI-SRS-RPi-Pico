...
# --- Maski bitowe OSW ---
# Zgodne z TX SWITCH_MAP: "Oswietlenie 1":(1,1) i "Oswietlenie 2":(1,2)
# _gui_ch[1] bit1 = OSW1 (mask 0x02), bit2 = OSW2 (mask 0x04)
OSW1_MASK = 0x02   # bit 1  →  0b00000010
OSW2_MASK = 0x04   # bit 2  →  0b00000100
...
# --- OSW: niezalezne stany (POPRAWKA — brak wzajemnego blokowania) ---
_osw1_last      = False
_osw2_last      = False
_osw1_last_sent = 0
_osw2_last_sent = 0
    ...
    while True:
        ...
        while pos + 3 < len(buffer):
            ...
            if buffer[pos + 2] == 0x16:
                ...
                # --- OSW (CH6) ---
                # POPRAWKA: kazdy bit sprawdzany niezaleznie przez maskowaie
                # Poprzedni blad: porownanie ch6 == 2/4/0 nie obsługiwało
                # przypadku gdy oba bity sa ustawione jednoczesnie (ch6 == 6)
                ch6      = _ch[5]
                osw1_now = bool(ch6 & OSW1_MASK)   # bit 1
                osw2_now = bool(ch6 & OSW2_MASK)   # bit 2

                # OSW1 — reaguje tylko na zmiane bitu 1, niezaleznie od OSW2
                if osw1_now != _osw1_last:
                    if time.ticks_diff(now, _osw1_last_sent) > OSW_COOLDOWN:
                        can.sendMsgBuf(CAN_ID_OSW1, 0, 8,
                                       OSW_ON if osw1_now else OSW_OFF)
                        sys.stdout.write(
                            '\x1b[11;0H>>> OSW1 {} <<<\n'.format(
                                "ON " if osw1_now else "OFF"))
                        _osw1_last      = osw1_now
                        _osw1_last_sent = now

                # OSW2 — reaguje tylko na zmiane bitu 2, niezaleznie od OSW1
                if osw2_now != _osw2_last:
                    if time.ticks_diff(now, _osw2_last_sent) > OSW_COOLDOWN:
                        can.sendMsgBuf(CAN_ID_OSW2, 0, 8,
                                       OSW_ON if osw2_now else OSW_OFF)
                        sys.stdout.write(
                            '\x1b[12;0H>>> OSW2 {} <<<\n'.format(
                                "ON " if osw2_now else "OFF"))
                        _osw2_last      = osw2_now
                        _osw2_last_sent = now
                ...
                if time.ticks_diff(now, last_display) >= 100:
                    ...
                    out += '----------------------------------------\n'
                    out += 'OSW1: {} (ch6={}) OSW2: {}\n'.format(
                        "ON " if _osw1_last else "OFF", ch6,
                        "ON " if _osw2_last else "OFF")
