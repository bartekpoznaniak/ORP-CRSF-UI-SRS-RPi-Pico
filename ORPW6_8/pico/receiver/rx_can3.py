...
OSW_COOLDOWN  = 400   # ms — musi byc > czasu mrugnięcia STM32
...
_fire_armed   = True
_fire_last    = 0
_osw_last_ch6  = -1   # edge detection — nieznany stan startowy
_osw_last_sent = 0
            ...
            if buffer[pos + 2] == 0x16:
                ...
                if ch6 != _osw_last_ch6:
                    if time.ticks_diff(now, _osw_last_sent) > OSW_COOLDOWN:
                        if ch6 == 2:
                            ...
                            sys.stdout.write('\x1b[11;0H>>> OSW1 ON  / OSW2 OFF <<<\n')
                        elif ch6 == 4:
                            can.sendMsgBuf(CAN_ID_OSW2, 0, 8, OSW_ON)
                            time.sleep_ms(5)
                            can.sendMsgBuf(CAN_ID_OSW1, 0, 8, OSW_OFF)
                            sys.stdout.write('\x1b[11;0H>>> OSW2 ON  / OSW1 OFF <<<\n')
                        elif ch6 == 0:
                            can.sendMsgBuf(CAN_ID_OSW1, 0, 8, OSW_OFF)
                            time.sleep_ms(5)
                            can.sendMsgBuf(CAN_ID_OSW2, 0, 8, OSW_OFF)
                            sys.stdout.write('\x1b[11;0H>>> OSW1 OFF / OSW2 OFF <<<\n')
                        _osw_last_ch6  = ch6
                        _osw_last_sent = now
                ...
                if time.ticks_diff(now, last_display) >= 100:
                    ...
                    for i in range(0, 16, 2):
                       ...
                       # out += f'CH{i+1:02}: {_ch[i]:<5} ({_ch[i]:08b}) | CH{i+2:02}: {_ch[i+1]:<5} ({_ch[i+1]:08b})\n' #binarnie
