#!/usr/bin/env python3
"""
CRSF Raw Logger - Enhanced Edition
Loguje ramki CRSF z weryfikacją CRC8, timestampami i statystykami
"""
...
def log_raw(frame, crc_valid):
    """Loguje ramkę z timestampem, typem i statusem CRC"""
    ...
    # Status CRC
    crc_status = "✓ CRC_OK" if crc_valid else "✗ CRC_FAIL"
    
    # Aktualizuj statystyki typów
    stats['frame_types'][type_name] = stats['frame_types'].get(type_name, 0) + 1
    
    print(f"[{timestamp}] ADDR:0x{address:02X} TYPE:{type_name:25} LEN:{len(frame):02} {crc_status:10} | {hex_data}")
...
def print_statistics():
    ...
    print(f"CRC poprawne:            {stats['crc_ok']} ({100*stats['crc_ok']/(stats['total_frames'] or 1):.1f}%)")
    print(f"CRC błędne:              {stats['crc_fail']} ({100*stats['crc_fail']/(stats['total_frames'] or 1):.1f}%)")
    print("\nRozkład typów ramek:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='CRSF Raw Logger z weryfikacją CRC i statystykami',
    ...
    try:
        ...
        while True:
            ...
            while len(buffer) >= 4:
                ...
                # Loguj ramkę
                log_raw(frame, crc_valid)
                
                # Usuń przetworzony pakiet z buffera
